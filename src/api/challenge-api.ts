...
export default {
  ...
  async filter(requestDto: any, list: any) {
    await request
      .post("/challenge/filter", requestDto)
      .then((response) => (list.value = response));
  },
  async compileChallengeCode(testCaseId: any, sourceCode: any, result: any) {
    await request
      .post("/challenge/submit-code-json/" + testCaseId, sourceCode)
      .then((response: any) => {
  ...
  async searchChallenge(challengeName: any) {
    return await request.post("/challenge/search-challenge/" + challengeName);
  },
};
